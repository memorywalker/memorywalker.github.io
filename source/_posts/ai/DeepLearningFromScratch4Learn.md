...
对函数$f(x) = 0.01x^2+0.1x$ 计算x为5的导数，使用数学分析的方案$\frac{dy}{dx}=0.02x+0.1$，当x=5时，得到微分值为0.2，和使用数值微分计算出来0.19999是相同的
...
这个公式有两个变量，所以有必要区分对哪个变量求导数，即对$x_0$和$x_1$两个变量中的哪一个求导数。
...
用图形表示元素值为负梯度的向量（导数值取负数），$f(x_0, x_1) = x_0^2 + x_1^2$的梯度呈现为有向向量（箭头）。梯度指向函数$f(x_0, x_1)$的“最低处”（最小值），就像指南针一样，所有的箭头都指向同一点。其次，我们发现离“最低处”越远，箭头越大。
