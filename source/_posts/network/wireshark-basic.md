...
默认写数据是异步的async WRITE Call，服务器在真正存盘之前就会应答WRITE Reply从而提高性能，只有COMMIT之后的数据才认为是写成功的。写操作中有`UNSTABLE`标志。

写操作中`FILE_SYNC`表示当前为同步sync写，同步写是一写一答，所以不需要COMMIT操作。一些客户端无论设置`wsize`为多少，每次写的数据都为4KB。

mount时使用`noac`选项表示让客户端不缓存文件属性，但是会把写操作设置为sync方式，导致效率降低
...
`telnet 192.168.1.101 111`查看portmap的111端口能否连接上
