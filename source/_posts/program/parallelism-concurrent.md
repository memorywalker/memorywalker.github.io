...
2025-07-31 更新：今天看到FastAPI官方的[学习指南](https://fastapi.tiangolo.com/zh/async/#_2)，讲解异步、并发和并行很直观，更新了自己的新理解。
...
python中使用`await`关键字告诉CPU程序执行到这里要等待一会儿，CPU可以去做点别的事情，等一会再回来。

`await`需要在`async def`定义的函数中使用，当调用一个`async def`定义的函数时也必须用`await`去等它